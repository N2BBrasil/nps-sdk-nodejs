...
SoapClient.prototype.callClient = function (service, params, callback) {
  ...
  soap.createClient(
    wsdl,
    { disableCache: cache },
    function handleRequest(err, client) {
      if (err) throw error;

      if (localConfig.cert) {
        client.setSecurity(
          new soap.ClientSSLSecurity(localConfig.certKey, localConfig.cert, {
...
SoapClient.prototype.log = function (xml, config) {
  if (config.logger) {
    if (
      config.environment == constants.PRODUCTION &&
      config.logLevel == constants.DEBUG
    ) {
      config.logger.error(
        "DEBUG level is not allowed on PRODUCTION ENVIRONMENT"
      );
    } else {
      var Util = new Utils();
...
SoapClient.prototype.addMerchantAdditionalDetails = function (args) {
  if (!args.psp_MerchantAdditionalDetails) {
    ...
    Object.assign(args.psp_MerchantAdditionalDetails, {
